...
void updateLeds() {
  ...
  // Update LED_WORKING based on battery status and leads connection
  if (!leadsConnected) {
    // Leads disconnected - fast blinking
    ledWorkingState = (currentMillis / 250) % 2;
  } else if (batteryVoltage <= BATTERY_WARNING_THRESHOLD) {
    // Warning battery - slow blinking (once per second)
  ...
  // LED_FETAL pulses with detected heartbeat
  if (currentMillis - lastFetalPeak < 150 && fetalBpm > 0) {
    digitalWrite(LED_FETAL, HIGH);
  } else {
    digitalWrite(LED_FETAL, LOW);
  }
