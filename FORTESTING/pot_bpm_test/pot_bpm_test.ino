...
#define LED_STATUS 27       // LED for status
...
const float MIN_PROMINENCE = 0.01;   // Minimum peak prominence (most sensitive)
const float MAX_PROMINENCE = 0.2;    // Maximum peak prominence (least sensitive)
const int MIN_DISTANCE_MS = 250;     // Minimum distance between peaks (240 BPM max)
const int MAX_DISTANCE_MS = 1200;    // Maximum distance between peaks (50 BPM min)

// BPM Range Constants
const int MIN_VALID_BPM = 40;      // Minimum valid BPM for any heart rate
const int MAX_VALID_BPM = 200;     // Maximum valid BPM for any heart rate
const int MIN_MATERNAL_BPM = 50;   // Minimum maternal heart rate
const int MAX_MATERNAL_BPM = 105;  // Maximum maternal heart rate
const int MIN_FETAL_BPM = 106;     // Minimum fetal heart rate
const int MAX_FETAL_BPM = 180;     // Maximum fetal heart rate

// Variables for potentiometer and sensitivity control
int potValue = 0;
float prominence = 0.1;         // Threshold for peak detection (adjusted by pot)
int minPeakDistance = 500;      // Minimum time between peaks (adjusted by pot)
...
const float PT_HP_CONSTANT = 0.995;  // High-pass filter constant
const float PT_LP_CONSTANT = 0.125;  // Low-pass filter constant
...
void loop() {
  ...
  // If leads are off, don't process ECG
  if (leadsOff) {
    return;
  }
  ...
  // Update LEDs for visual feedback
  updateLeds();
...
bool checkLeadsOff() {
  ...
  // Update status LED
  if (!leadsConnected) {
    digitalWrite(LED_STATUS, (millis() / 250) % 2); // Blink for lead-off
  } else {
    digitalWrite(LED_STATUS, LOW); // Solid for connected
  }
...
float filterEcg(int rawEcg) {
  ...
  for (int i = 0; i < BUFFER_SIZE; i++) {
    float weight = exp(-pow(i - BUFFER_SIZE/2.0, 2) / (2 * pow(BUFFER_SIZE/4.0, 2)));
  ...
  baseline = 0.995 * baseline + 0.005 * filtered;
...
bool panTompkinsQRS(float input_data) {
    ...
    pt_der_data = (2 * pt_lp_data + der_data[0] - der_data[2] - 2 * der_data[3]) / 8.0;
    ...
    // Update thresholds
    pt_threshold_i1 = pt_npk + 0.25 * (pt_peak - pt_npk);
    pt_threshold_i2 = 0.5 * pt_threshold_i1;
    
    // Check for QRS complex
    if(pt_mwi_data > pt_threshold_i1) {
        if(current_time - last_qrs > 200) {  // Refractory period
            pt_peak = 0.875 * pt_peak + 0.125 * pt_mwi_data;
            qrs_detected = true;
            ...
            if(last_qrs > 0) {
                ...
                if(rr_count < 8) {
                    ...
                    if(rr_interval >= 0.92 * rr_average2 && rr_interval <= 1.16 * rr_average2) {
                        rr_average2 = 0.875 * rr_average2 + 0.125 * rr_interval;
                    }
                }
            }
        ...
        pt_npk = 0.875 * pt_npk + 0.125 * pt_mwi_data;
    }
    
    // Search back for missed QRS
    if(current_time - last_qrs > 1.66 * rr_average2) {
        pt_threshold_i1 *= 0.5;
        pt_threshold_i2 *= 0.5;
        pt_rr_missed++;
 ...
 */
void updateLeds() {
  // Fetal LED pulses with each detected fetal peak
  ...
  // Maternal LED pulses with each detected maternal peak
  if (millis() - lastMaternalPeak < 150 && maternalBpm > 0) {
    digitalWrite(LED_MATERNAL, HIGH);
  } else {
    digitalWrite(LED_MATERNAL, LOW);
  }
...
void displayInfo(int rawEcg, float filteredEcg) {
  ...
  // Output maternal BPM (show 0 if no recent detection)
  if (maternalBpm > 0 && millis() - lastMaternalPeak < 5000) {
  ...
  // Print detailed BPM information to Serial Monitor every second
