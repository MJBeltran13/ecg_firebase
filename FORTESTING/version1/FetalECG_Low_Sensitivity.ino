...
// These settings are designed for clear, reliable detection of strong fetal signals
// Appropriate for: Healthy pregnancies, minimal interference, good electrode placement
// Less likely to produce false positives but may miss weaker signals
const float FETAL_PROMINENCE = 0.20;            // Higher threshold, requires stronger peaks
const int FETAL_MIN_DISTANCE = SAMPLING_RATE * 0.5;  // ~120 BPM max
const float FETAL_WEIGHT = 0.03;                // Lower weight for high-frequency components
...
// Device information
String deviceId = "esp32_fetal_ecg_low";
unsigned long readingCounter = 0;

void setup() {
  Serial.begin(115200);
  Serial.println("Fetal ECG Monitor - LOW SENSITIVITY Mode");
...
void detectPeaks(float filteredEcg, bool isFetal) {
  ...
  if (isFetal) {
    ...
    fetalMaxValue *= 0.995; // Slower decay for low sensitivity
    fetalMinValue *= 1.005; // Slower rise for low sensitivity
  } else {
