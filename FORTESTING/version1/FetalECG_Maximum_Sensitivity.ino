...
// These settings are designed for detecting extremely weak fetal signals
// Appropriate for: Early pregnancy, suboptimal electrode placement, weak signals
// Will detect almost any potential fetal peak, but higher chance of false positives
const float FETAL_PROMINENCE = 0.05;            // Very low threshold, extremely sensitive
const int FETAL_MIN_DISTANCE = SAMPLING_RATE * 0.3;  // ~200 BPM max
const float FETAL_WEIGHT = 0.08;                // Very high weight for high-frequency components

// Enhanced filtering for maximum sensitivity
const int FETAL_FILTER_ITERATIONS = 2;          // Apply filter twice for stronger effect
...
// Additional fetal signal enhancement
float fetalHistory[5] = {0, 0, 0, 0, 0};
int fetalHistoryIndex = 0;

// Device information
String deviceId = "esp32_fetal_ecg_max";
unsigned long readingCounter = 0;

void setup() {
  Serial.begin(115200);
  Serial.println("Fetal ECG Monitor - MAXIMUM SENSITIVITY Mode");
  Serial.println("WARNING: May detect false positives - use for difficult signals only");
...
void loop() {
  ...
  // Apply enhanced filtering for fetal signals (multi-pass filtering)
  float fetalFiltered = bandpassFilter(rawEcg, true);
  for (int i = 1; i < FETAL_FILTER_ITERATIONS; i++) {
    fetalFiltered = bandpassFilter(fetalFiltered * 5, true); // Amplify between iterations
  }
  
  // Additional signal enhancement for fetal signal
  fetalHistory[fetalHistoryIndex] = fetalFiltered;
  fetalHistoryIndex = (fetalHistoryIndex + 1) % 5;
  
  // Calculate enhanced fetal signal (differential boost)
  float enhancedFetal = fetalFiltered;
  for (int i = 0; i < 5; i++) {
    if (i != fetalHistoryIndex) {
      // Emphasize differences from recent history
      enhancedFetal += (fetalFiltered - fetalHistory[i]) * 0.5;
    }
  }
  
  // Detect peaks in filtered signals
  detectPeaks(maternalFiltered, false);  // Maternal
  detectPeaks(enhancedFetal, true);      // Enhanced fetal
  ...
  if (millis() - lastDisplayTime >= 1000) {
    ...
    sendToFirebase(rawEcg, enhancedFetal, maternalFiltered);
  }
...
void detectPeaks(float filteredEcg, bool isFetal) {
  ...
  if (isFetal) {
    ...
    // Fetal peak detection with maximum sensitivity
    if (filteredEcg > threshold && !fetalPeakDetected &&
      ...
      if (lastFetalPeak != 0) {
        ...
        // Very wide acceptable range for maximum sensitivity
        if (fetalBpm < 80 || fetalBpm > 220) {
          fetalBpm = 0; // Invalid reading
        }
      }
      lastFetalPeak = currentTime;
      
      // Flash LED for more visible indication (longer for stronger peaks)
      int ledTime = 200 + (int)((filteredEcg - threshold) * 10);
      ledTime = constrain(ledTime, 200, 500);
    } else if (filteredEcg < threshold * 0.6) { // Much lower reset threshold for maximum sensitivity
      fetalPeakDetected = false;
    }
    
    // Very fast decay for maximum sensitivity - adapts rapidly to signal changes
    fetalMaxValue *= 0.97;
    fetalMinValue *= 1.03;
  } else {
