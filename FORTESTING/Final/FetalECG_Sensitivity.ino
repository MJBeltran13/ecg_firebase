...
#define SENSITIVITY_PIN 34  // Analog pin for sensitivity adjustment
...
// FETAL_PROMINENCE: Lower = more sensitive (detects smaller peaks)
// FETAL_MIN_DISTANCE: Lower = allows faster heart rates to be detected
// FETAL_WEIGHT: Higher = amplifies high-frequency components (fetal signals)
const float SENSITIVITY_LEVELS[5][3] = {
  // {FETAL_PROMINENCE, FETAL_MIN_DISTANCE, FETAL_WEIGHT}
  {0.20, SAMPLING_RATE * 0.5, 0.03},  // Level 1: Least sensitive
  {0.15, SAMPLING_RATE * 0.45, 0.04}, // Level 2: Low sensitivity
  {0.10, SAMPLING_RATE * 0.4, 0.05},  // Level 3: Medium sensitivity (default)
  {0.07, SAMPLING_RATE * 0.35, 0.06}, // Level 4: High sensitivity
  {0.05, SAMPLING_RATE * 0.3, 0.08}   // Level 5: Maximum sensitivity
};

// Current sensitivity level (1-5)
int currentSensitivityLevel = 2;  // Start at level 3 (medium)

// Peak detection parameters for maternal signal (fixed)
...
// Device information
String deviceId = "esp32_fetal_ecg";
...
void checkSensitivityLevel();
void displaySensitivityLevel();

void setup() {
  Serial.begin(115200);
  Serial.println("Fetal ECG Monitor with Adjustable Sensitivity");
  ...
  // Display initial sensitivity level
  displaySensitivityLevel();
}

void loop() {
  // Check if sensitivity level needs adjustment
  checkSensitivityLevel();
  ...
  if (millis() - lastDisplayTime >= 1000) {
    ...
    Serial.println(currentSensitivityLevel + 1);  // 1-indexed for display
...
float bandpassFilter(int rawEcg, bool isFetalFilter) {
  ...
  for (int i = 0; i < BUFFER_SIZE; i++) {
    ...
    if (isFetalFilter) {
      ...
      // Use sensitivity-adjusted weight
      weight = SENSITIVITY_LEVELS[currentSensitivityLevel][2] * 
...
void detectPeaks(float filteredEcg, bool isFetal) {
  ...
  if (isFetal) {
    ...
    float prominence = SENSITIVITY_LEVELS[currentSensitivityLevel][0];
    int minDistance = SENSITIVITY_LEVELS[currentSensitivityLevel][1];
    
    float threshold = fetalMinValue + (fetalMaxValue - fetalMinValue) * prominence;
    
    // Fetal peak detection
    if (filteredEcg > threshold && !fetalPeakDetected &&
        (currentTime - lastFetalPeak) >= minDistance &&
...
bool sendToFirebase(int rawEcg, float fetalFiltered, float maternalFiltered) {
  ...
  doc["sensitivityLevel"] = currentSensitivityLevel + 1;  // 1-indexed for display
...
/**
 * Check analog input to determine sensitivity level
 * Divides the ADC range (0-4095) into 5 equal parts
 */
void checkSensitivityLevel() {
  static unsigned long lastCheckTime = 0;
  
  // Only check every 500ms to avoid rapid changes
  if (millis() - lastCheckTime >= 500) {
    lastCheckTime = millis();
    
    // Read from sensitivity adjustment potentiometer
    int sensorValue = analogRead(SENSITIVITY_PIN);
    
    // Map the analog reading (0-4095) to sensitivity levels (0-4)
    int newLevel = map(sensorValue, 0, 4095, 0, 4);
    
    // Update sensitivity level if changed
    if (newLevel != currentSensitivityLevel) {
...
/**
 * Display current sensitivity level on Serial monitor
 */
void displaySensitivityLevel() {
  Serial.println("\n--- SENSITIVITY LEVEL CHANGED ---");
  Serial.print("Current Level: ");
  Serial.print(currentSensitivityLevel + 1);  // 1-indexed for display
  Serial.print(" of 5 (");
  
  // Describe the current sensitivity level
  switch (currentSensitivityLevel) {
    case 0:
      Serial.println("Least Sensitive)");
      break;
    case 1:
    ...
    case 3:
      ...
      break;
    case 4:
      Serial.println("Maximum Sensitivity)");
      break;
  }
