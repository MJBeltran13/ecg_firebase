...
// These settings are designed for detecting weak fetal signals
// Appropriate for: Earlier pregnancy stages, challenging signal conditions
// May detect more peaks but potentially more false positives
const float FETAL_PROMINENCE = 0.07;            // Lower threshold, more sensitive
const int FETAL_MIN_DISTANCE = SAMPLING_RATE * 0.35;  // ~170 BPM max
const float FETAL_WEIGHT = 0.06;                // Higher weight for high-frequency components
...
void setup() {
  Serial.begin(115200);
  Serial.println("Fetal ECG Monitor - HIGH SENSITIVITY Mode");
...
void detectPeaks(float filteredEcg, bool isFetal) {
  ...
  if (isFetal) {
    ...
    // Fetal peak detection with high sensitivity
    if (filteredEcg > threshold && !fetalPeakDetected &&
      ...
      if (lastFetalPeak != 0) {
        ...
        // Wider acceptable range for high sensitivity
        if (fetalBpm < 90 || fetalBpm > 200) {
          fetalBpm = 0; // Invalid reading
        }
      }
      lastFetalPeak = currentTime;
    } else if (filteredEcg < threshold * 0.7) { // Lower reset threshold for more sensitivity
      fetalPeakDetected = false;
    }
    
    // Faster decay for high sensitivity - adapts quicker to signal changes
    fetalMaxValue *= 0.985;
    fetalMinValue *= 1.015;
  } else {
